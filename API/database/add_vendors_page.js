...
connection.connect((err) => {
    ...
    const insertPageSQL = `
        ...
        VALUES ('Vendors', '/vendors', 'truck', 1, 2, 6, NOW(), NOW())
    `;

    connection.query(insertPageSQL, (err, result) => {
        if (err) {
            console.error('Error adding Vendors page:', err);
            connection.end();
            process.exit(1);
        }
        console.log('Vendors page added successfully with ID:', result.insertId);
        connection.end();
        process.exit(0);
    });
});
