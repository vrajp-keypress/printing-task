...
async function ensureSchema() {
    ...
    const checkSql = `
        ...
        WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'customers' AND COLUMN_NAME IN ('password','created_at','updated_at','balance','token')
    `;
    const required = new Set(['password', 'created_at', 'updated_at', 'balance', 'token']);
    ...
    try {
        ...
        if (missing.has('token')) alters.push('ADD COLUMN `token` TEXT NULL AFTER `password`');
        if (alters.length) {
