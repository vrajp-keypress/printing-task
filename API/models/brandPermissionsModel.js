...
const BrandPermissions = {
  ...
  getByRole: async (roleId) => {
    try {
      const [results] = await db.execute(`
        SELECT permissions.*, pages.pagename as pageName, pages.url as pageUrl, page_categories.name as categoryName
        FROM permissions
        LEFT JOIN pages ON permissions.pageId = pages.pageId
        LEFT JOIN page_categories ON pages.categoryId = page_categories.id
        WHERE permissions.roleId = ?
        ORDER BY page_categories.displayOrder ASC, pages.displayOrder ASC
      `, [roleId]);
      return {
        status: 'success',
        data: results
